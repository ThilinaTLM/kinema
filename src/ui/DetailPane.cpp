    ...
    , m_loader(loader)
{
    ...
    // ---- Series picker (hidden for movies) --------------------------------
    m_pickerEmpty = new QWidget(this);
    m_seriesPicker = new SeriesPicker(m_loader, this);
    connect(m_seriesPicker, &SeriesPicker::episodeActivated,
        this, &DetailPane::episodeSelected);
    m_pickerStack = new QStackedWidget(this);
    m_pickerStack->addWidget(m_pickerEmpty);
    m_pickerStack->addWidget(m_seriesPicker);
    m_pickerStack->setCurrentWidget(m_pickerEmpty);
    ...
    auto* pickerWrap = new QVBoxLayout;
    pickerWrap->setContentsMargins(12, 0, 12, 6);
    pickerWrap->addWidget(m_pickerStack);

    auto* root = new QVBoxLayout(this);
    root->setContentsMargins(0, 0, 0, 0);
    root->setSpacing(4);
    root->addWidget(m_metaStack, 0);
    root->addLayout(pickerWrap, 0);
...
void DetailPane::setMeta(const api::MetaDetail& meta)
{
    ...
    // Movie mode: hide the picker.
    m_seriesPicker->clear();
    m_pickerStack->setCurrentWidget(m_pickerEmpty);
}

void DetailPane::setSeries(const api::SeriesDetail& series)
{
    setMeta(series.meta);
    m_seriesPicker->setSeries(series);
    m_pickerStack->setCurrentWidget(m_seriesPicker);
}
