...
{
public:
    KinemaImageResponse(ui::ImageLoader* loader, const QUrl& url) : m_loader(loader), m_url(url)
    {
        ...
        m_conn = QObject::connect(
            loader,
            &ui::ImageLoader::posterReady,
            this,
            [this](const QUrl& finishedUrl) {
...
KinemaImageProvider::KinemaImageProvider(ui::ImageLoader* loader) : m_loader(loader) { }

KinemaImageProvider::~KinemaImageProvider() = default;

QQuickImageResponse* KinemaImageProvider::requestImageResponse(const QString& id,
                                                               const QSize& /*requestedSize*/)
{
    ...
    const auto src = QUrl::fromPercentEncoding(
        q.queryItemValue(QStringLiteral("u"), QUrl::FullyEncoded).toUtf8());
    const QUrl url(src);
