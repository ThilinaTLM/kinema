...
{
    return QStringLiteral(":/qt/qml/dev/tlmtech/kinema/app/ui/qml/icons/lucide/") + iconId
           + QStringLiteral(".svg");
}
...
AppIconResolver::AppIconResolver(QObject* parent) : QObject(parent) { }

QString AppIconResolver::url(const QString& iconId, const QString& hexColor) const
{
    const QString path = ensureIconFile(iconId, hexColor);
    return path.isEmpty() ? QString{} : QUrl::fromLocalFile(path).toString();
}

QString AppIconResolver::ensureIconFile(const QString& iconId, const QString& hexColor) const
{
    if (iconId.isEmpty()) {
        return {};
    }
    ...
    const QString fileName = iconId + QStringLiteral("-") + color + QStringLiteral(".svg");
