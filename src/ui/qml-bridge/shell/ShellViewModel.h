 ...
 * to live on `ShellViewModel`: drawer actions, keyboard-shortcut
...
public:
    explicit ShellViewModel(ShellDependencies dependencies, QObject* parent = nullptr);
    ...
    static CloseDecision
    evaluateCloseRequest(bool reallyQuit, bool closeToTrayPref, bool trayAvail, bool toastShown);
...
public Q_SLOTS:
    ...
    void pushSubtitlesPage(const domain::PlaybackContext& ctx, bool fromPlayer);
    ...
    /// `text`.
    void copyToClipboard(const QString& text, const QString& confirmMessage = QString());
    ...
    /// streams shortcut.
    void findMovieStreamsByImdb(const QString& imdbId, const QString& title);
    void findSeriesStreamsByImdb(const QString& imdbId, const QString& title);
    ...
    void openSeriesDetailAt(const QString& imdbId, const QString& title, int season, int episode);
...
private:
    ...
    void openEmbeddedPlayer(const QUrl& url, const domain::PlaybackContext& ctx);
    ...
    ShellDependencies m_dependencies;
