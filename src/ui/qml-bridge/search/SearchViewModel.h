...
Q_SIGNALS:
    ...
    /// Routed by `ShellViewModel` to a passive notification (phase
    ...
    void findMovieStreamsByImdbRequested(const QString& imdbId, const QString& title);
    void findSeriesStreamsByImdbRequested(const QString& imdbId, const QString& title);

    /// Forwarded into `ShellViewModel::passiveMessage` so the user
...
private:
    ...
    lookupExactImdb(QString imdbId, domain::MediaKind preferredKind);
    QCoro::Task<QList<domain::MetaSummary>> searchTmdbFallback(domain::MediaKind kind,
                                                               QString text);
