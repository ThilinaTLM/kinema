...
{
    if (e.key.kind != domain::MediaKind::Series || !e.key.season.has_value()
        || !e.key.episode.has_value()) {
        return {};
    }
...
QString secondaryLineFor(const domain::HistoryEntry& e)
{
    ...
    return i18nc(
        "@info library rail, episode code dot title", "%1 \u00b7 %2", code, e.episodeTitle);
}
...
QString tertiaryLineFor(double progressFraction)
{
    ...
    const int percent = qRound(qBound(0.0, progressFraction, 1.0) * 100.0);
    return i18nc("@info continue-watching, resume percent", "Resume from %1%", percent);
}
...
ContinueWatchingViewModel::ContinueWatchingViewModel(
    ...
    : QObject(parent), m_history(history), m_model(new LibraryRailModel(this))
{
    if (m_history) {
        connect(m_history,
