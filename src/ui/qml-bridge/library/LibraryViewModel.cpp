...
LibraryViewModel::LibraryViewModel(controllers::LibraryController* library,
    ...
    , m_recentlyAddedModel(new LibraryRailModel(this))
{
    if (m_library) {
        connect(
            m_library, &controllers::LibraryController::changed, this, &LibraryViewModel::refresh);
        connect(m_library,
                &controllers::LibraryController::statusMessage,
                this,
                &LibraryViewModel::statusMessage);
    }
    if (m_watched) {
        ...
        connect(
            m_watched, &controllers::WatchedController::changed, this, &LibraryViewModel::refresh);
    }
    refresh();
}
...
{
    return m_kind != KindFilter::All || m_status != StatusFilter::All || !m_genreIds.isEmpty()
           || m_sort != SortMode::RecentlyAdded || m_dateWindow != DateWindowAny
           || m_minRatingPct > 0 || m_hideWatched;
}
...
{
    bool changed = false;
    if (m_kind != KindFilter::All) {
        m_kind = KindFilter::All;
        changed = true;
    }
    ...
    if (m_sort != SortMode::RecentlyAdded) {
        m_sort = SortMode::RecentlyAdded;
        changed = true;
    }
    if (m_dateWindow != DateWindowAny) {
...
void LibraryViewModel::activate(int row)
{
    ...
    if (r->season && r->episode) {
        Q_EMIT openSeriesEpisodeRequested(r->imdbId, r->title, *r->season, *r->episode);
        return;
    }
...
void LibraryViewModel::findStreamsForRow(int row)
{
    ...
    if (!r)
        return;
    if (r->upcoming) {
        Q_EMIT statusMessage(i18nc("@info:status", "\u201c%1\u201d hasn't released yet.", r->title),
                             4000);
        return;
    }
    if (r->kind == domain::MediaKind::Movie) {
    ...
    if (r->season && r->episode) {
        Q_EMIT openSeriesEpisodeStreamsRequested(r->imdbId, r->title, *r->season, *r->episode);
        return;
    }
...
void LibraryViewModel::activateRail(const QString& railId, int row)
{
    ...
    if (railId == QLatin1String("upNext"))
        m = m_upNextModel;
    else if (railId == QLatin1String("airingSoon"))
        m = m_airingSoonModel;
    else if (railId == QLatin1String("recentlyAdded"))
        m = m_recentlyAddedModel;
    if (!m)
        return;
...
void LibraryViewModel::findStreamsForRailRow(const QString& railId, int row)
{
    LibraryRailModel* m = nullptr;
    if (railId == QLatin1String("upNext"))
        m = m_upNextModel;
...
void LibraryViewModel::openRailRowDetail(const QString& railId, int row)
{
    LibraryRailModel* m = nullptr;
    if (railId == QLatin1String("upNext"))
        m = m_upNextModel;
...
void LibraryViewModel::markRailRowWatched(const QString& railId, int row)
{
    LibraryRailModel* m = nullptr;
    if (railId == QLatin1String("upNext"))
        m = m_upNextModel;
    ...
    if (r->kind == domain::MediaKind::Movie) {
        ...
        Q_EMIT statusMessage(i18nc("@info:status", "Marked \u201c%1\u201d as watched.", r->title),
                             3000);
        return;
    }
    if (r->season && r->episode) {
        m_watched->setEpisodeWatched(r->imdbId, *r->season, *r->episode, /*watched=*/true);
        Q_EMIT statusMessage(i18nc("@info:status", "Marked episode as watched."), 3000);
        return;
    }
...
void LibraryViewModel::buildEntries()
{
    ...
    for (const auto& t : titles) {
        ...
        if (t.kind == domain::MediaKind::Movie) {
            const bool watched = m_watched && m_watched->isMovieWatched(t.imdbId);
            const auto resume = m_watched ? m_watched->resumeEntryForMovie(t.imdbId)
                                          : std::optional<domain::HistoryEntry>{};
            const double progress = m_watched ? m_watched->movieProgress(t.imdbId) : -1.0;
            e.resume = resume;
        ...
        for (const auto& ep : e.episodes) {
            ...
            ++e.totalAired;
            const bool epWatched =
                m_watched && m_watched->isEpisodeWatched(t.imdbId, ep.season, ep.episode);
            if (epWatched) {
                ++e.watchedAired;
            ...
            const auto resume =
                m_watched ? m_watched->resumeEntryForEpisode(t.imdbId, ep.season, ep.episode)
                          : std::optional<domain::HistoryEntry>{};
            if (resume && (!e.resume || resume->lastWatchedAt > e.resume->lastWatchedAt)) {
                e.resume = resume;
...
void LibraryViewModel::rebuildAvailableGenres()
{
    ...
    for (const auto& e : m_entries) {
        for (const auto& g : e.title.genres) {
            ...
            if (trimmed.isEmpty() || byName.contains(trimmed))
                continue;
...
void LibraryViewModel::rebuildRails()
{
    ...
    const auto today = QDate::currentDate();
    const auto episodeHorizon = today.addDays(kAiringSoonEpisodeHorizonDays);
    ...
    for (const auto& e : m_entries) {
        ...
        if (t.kind == domain::MediaKind::Movie) {
            if (t.releaseDate && *t.releaseDate > today) {
                ...
                row.tertiaryLine = i18nc("@info library airing-soon rail, movie release date",
                                         "Releases %1",
                                         releaseText(t.releaseDate));
                row.releaseDate = t.releaseDate;
                airingOrComing.append(std::move(row));
            }
        } else if (e.firstUpcoming && e.firstUpcoming->releaseDate
            ...
            row.secondaryLine = ep.title.isEmpty()
                                    ? code
                                    : i18nc("@info library rail, episode code dot title",
                                            "%1 \u00b7 %2",
                                            code,
                                            ep.title);
            row.tertiaryLine = i18nc("@info library airing-soon rail, episode air date",
                                     "Airs %1",
                                     releaseText(ep.releaseDate));
        ...
        // title.
        if (t.kind == domain::MediaKind::Series && e.nextToWatch && !e.resume.has_value()) {
    ...
    std::sort(byAdded.begin(), byAdded.end(), [](const Entry* a, const Entry* b) {
    ...
    std::sort(upNext.begin(), upNext.end(), [](const auto& a, const auto& b) {
        return QString::localeAwareCompare(a.title, b.title) < 0;
    });
    const auto byReleaseAsc = [](const LibraryRailRow& a, const LibraryRailRow& b) {
        if (a.releaseDate && b.releaseDate) {
            return *a.releaseDate < *b.releaseDate;
        }
        return a.releaseDate.has_value() > b.releaseDate.has_value();
    };
    std::sort(airingOrComing.begin(), airingOrComing.end(), byReleaseAsc);
...
bool LibraryViewModel::entryMatchesFilters(const Entry& e) const
{
    ...
    if (!m_genreIds.isEmpty()) {
        ...
        for (const auto& g : e.title.genres) {
            ...
            if (it != m_genreIdByName.constEnd() && m_genreIds.contains(it.value())) {
                any = true;
                break;
            }
        }
    ...
    if (m_minRatingPct > 0) {
        if (!e.title.imdbRating)
            return false;
        if (*e.title.imdbRating * 10.0 < m_minRatingPct)
            return false;
    }

    if (const auto floor = dateWindowFloor(m_dateWindow)) {
        if (!e.title.releaseDate)
            return false;
        if (*e.title.releaseDate < *floor)
            return false;
    }
...
LibraryListRow LibraryViewModel::toGridRow(const Entry& e) const
{
    ...
    case ResolvedStatus::Continue: {
        ...
        } else if (e.resume && e.resume->key.season && e.resume->key.episode) {
    ...
    case ResolvedStatus::ToWatch: {
        if (e.title.kind == domain::MediaKind::Movie) {
            ...
            const auto code = episodeCode(e.nextToWatch->season, e.nextToWatch->episode);
            row.subtitle =
                e.nextToWatch->title.isEmpty()
                    ? i18nc("@info library card subtitle, episode code", "Next: %1", code)
    ...
    case ResolvedStatus::Watched: {
        ...
        if (e.title.kind == domain::MediaKind::Movie) {
            ...
            row.progress =
                e.totalAired > 0 ? static_cast<double>(e.watchedAired) / e.totalAired : 1.0;
            ...
            row.subtitle =
                e.upcomingCount == 1
                    ? i18nc("@info library card subtitle",
                            "%1 airs %2",
                            episodeCode(e.firstUpcoming->season, e.firstUpcoming->episode),
                            epDate)
...
void LibraryViewModel::rebuildGrid()
{
    ...
    switch (m_sort) {
    case SortMode::Title:
        std::sort(matched.begin(), matched.end(), byTitle);
        break;
    case SortMode::ReleaseDate:
        std::sort(matched.begin(), matched.end(), byReleaseDesc);
        break;
    case SortMode::Rating:
        std::sort(matched.begin(), matched.end(), byRatingDesc);
        break;
    case SortMode::RecentlyAdded:
    default:
        std::sort(matched.begin(), matched.end(), byAddedDesc);
        break;
    }
