...
std::optional<Enum> safeEnumCast(int value, std::initializer_list<Enum> valid)
{
    for (auto v : valid) {
...
BrowseViewModel::BrowseViewModel(api::TmdbClient* tmdb,
    ...
    , m_tmdb(tmdb)
    , m_settings(settings)
    , m_results(new DiscoverSectionModel(i18nc("@title browse rail", "Browse"), this))
{
    restoreFromSettings();
...
{
    return m_tmdbConfigured && !m_authFailed && m_results->rowCount() > 0 && m_totalPages > 0
           && m_nextPage <= m_totalPages && !m_loading;
}
...
void BrowseViewModel::setLibraryController(controllers::LibraryController* lib)
{
    m_library = lib;
}

void BrowseViewModel::setWatchedController(controllers::WatchedController* watched)
{
    m_watched = watched;
}
...
{
    bool& loaded = kind == domain::MediaKind::Movie ? m_genresLoadedMovie : m_genresLoadedSeries;
    bool& loading = kind == domain::MediaKind::Movie ? m_genresLoadingMovie : m_genresLoadingSeries;
    if (loading) {
        co_return;
    }
    if (loaded) {
        ...
        if (kind == m_kind) {
            ...
            for (int id : m_genreIds) {
                const bool stillThere =
                    std::any_of(m_availableGenres.cbegin(),
                                m_availableGenres.cend(),
                                [id](const domain::TmdbGenre& g) { return g.id == id; });
                if (stillThere) {
                    filtered.append(id);
                }
            }
