...
class MovieDetailViewModel : public QObject
{
    ...
    Q_PROPERTY(QString uiResolutionFilter READ uiResolutionFilter WRITE setUiResolutionFilter NOTIFY
                   uiFiltersChanged)
    Q_PROPERTY(bool uiHdrOnly READ uiHdrOnly WRITE setUiHdrOnly NOTIFY uiFiltersChanged)
    Q_PROPERTY(bool uiDolbyVisionOnly READ uiDolbyVisionOnly WRITE setUiDolbyVisionOnly NOTIFY
                   uiFiltersChanged)
    Q_PROPERTY(bool uiMultiAudioOnly READ uiMultiAudioOnly WRITE setUiMultiAudioOnly NOTIFY
                   uiFiltersChanged)
...
public:
    ...
    int sortMode() const noexcept { return m_streamState->sortMode(); }
    void setSortMode(int mode);
    bool sortDescending() const noexcept { return m_streamState->sortDescending(); }
    void setSortDescending(bool desc);
    bool debridConfigured() const noexcept { return !m_rdToken.isEmpty() || !m_adApiKey.isEmpty(); }
    int rawStreamsCount() const noexcept { return m_streamState->rawStreamsCount(); }

    QString uiResolutionFilter() const { return m_streamState->resolutionFilter(); }
    void setUiResolutionFilter(const QString& res);
    bool uiHdrOnly() const noexcept { return m_streamState->hdrOnly(); }
    void setUiHdrOnly(bool on);
    bool uiDolbyVisionOnly() const noexcept { return m_streamState->dolbyVisionOnly(); }
    void setUiDolbyVisionOnly(bool on);
    bool uiMultiAudioOnly() const noexcept { return m_streamState->multiAudioOnly(); }
...
public Q_SLOTS:
    ...
    /// `ShellViewModel` forwards as `showStreamsRequested(this)`.
    void requestStreams();
    ...
    /// `ShellViewModel`. The per-row variant carries the chosen
...
Q_SIGNALS:
    ...
    /// Forwarded into `ShellViewModel::passiveMessage`.
    ...
    /// carry IMDB ids); `ShellViewModel` routes them through a
    ...
    /// `ShellViewModel` with a passive notification.
    void subtitlesRequested(const domain::PlaybackContext& ctx);

    /// Emitted from `requestStreams()`. `ShellViewModel` connects
...
private:
    ...
    QCoro::Task<void>
    loadStreamsTask(QString imdbId, std::optional<QDate> released, quint64 expectedEpoch);
