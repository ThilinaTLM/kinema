...
#include "ui/qml-bridge/details/TitleActions.h"
#include "ui/qml-bridge/discover/DiscoverSectionModel.h"
#include "ui/qml-bridge/streams/StreamActionDispatch.h"
...
QString episodeDisplayLabel(const QString& seriesTitle, const domain::Episode& ep)
{
...
SeriesDetailViewModel::SeriesDetailViewModel(api::CinemetaClient* cinemeta,
                                             ...
                                             QObject* parent)
    : SeriesDetailViewModel(cinemeta,
                            indexers,
                            tmdb,
                            playback,
                            streamActions,
...
SeriesDetailViewModel::SeriesDetailViewModel(api::CinemetaClient* cinemeta,
    ...
    , m_streamState(new StreamListState(m_settings.filter(), domain::MediaKind::Series, this))
    , m_streams(m_streamState->model())
    , m_episodes(new EpisodesListModel(this))
    , m_similar(
          new DiscoverSectionModel(i18nc("@label series detail rail", "More like this"), this))
{
    connect(
        m_streamState, &StreamListState::sortChanged, this, &SeriesDetailViewModel::sortChanged);
    connect(m_streamState,
            &StreamListState::rawStreamsCountChanged,
            this,
            &SeriesDetailViewModel::rawStreamsCountChanged);
    connect(m_streamState,
            &StreamListState::uiFiltersChanged,
            this,
            &SeriesDetailViewModel::uiFiltersChanged);

    if (m_library) {
        connect(m_library,
                &controllers::LibraryController::changed,
                this,
                &SeriesDetailViewModel::refreshLibraryState);
    }
    if (m_watched) {
        connect(m_watched,
                &controllers::WatchedController::changed,
                this,
                &SeriesDetailViewModel::refreshEpisodeWatchedState);
    }
    ...
    if (m_tokens) {
        const auto onDebridChanged = [this](const QString&) {
            ...
            m_streamState->rebuildVisibleStreams();
        };
        connect(
            m_tokens, &controllers::TokenController::realDebridTokenChanged, this, onDebridChanged);
        connect(
            m_tokens, &controllers::TokenController::allDebridApiKeyChanged, this, onDebridChanged);
    }
}
...
{
    m_streamState->setSortMode(mode);
}

void SeriesDetailViewModel::setSortDescending(bool descending)
{
    m_streamState->setSortDescending(descending);
}

void SeriesDetailViewModel::setUiResolutionFilter(const QString& resolution)
{
    m_streamState->setResolutionFilter(resolution);
}

void SeriesDetailViewModel::setUiHdrOnly(bool enabled)
{
    m_streamState->setHdrOnly(enabled);
}

void SeriesDetailViewModel::setUiDolbyVisionOnly(bool enabled)
{
    m_streamState->setDolbyVisionOnly(enabled);
}

void SeriesDetailViewModel::setUiMultiAudioOnly(bool enabled)
{
    m_streamState->setMultiAudioOnly(enabled);
}

bool SeriesDetailViewModel::uiAnyFilterActive() const noexcept
{
    return m_streamState->anyUiFilterActive();
}

void SeriesDetailViewModel::clearUiFilters()
{
    m_streamState->clearUiFilters();
}
...
void SeriesDetailViewModel::applyMeta(const domain::SeriesDetail& sd)
{
    ...
    m_releaseDateText =
        (s.released && s.released->isValid()) ? core::formatReleaseDate(*s.released) : QString();
    Q_EMIT metaChanged();
    refreshLibraryState();
...
void SeriesDetailViewModel::rebuildSeasons()
{
    ...
    for (int n : std::as_const(m_seasonNumbers)) {
        m_seasonLabels.append(i18nc("@label series detail season tab", "Season %1", n));
    }
    Q_EMIT seasonsChanged();
    ...
    if (!m_seasonNumbers.isEmpty()) {
        newIdx = (m_currentSeasonIdx >= 0 && m_currentSeasonIdx < m_seasonNumbers.size())
                     ? m_currentSeasonIdx
                     : 0;
    }
...
{
    if (m_currentSeasonIdx < 0 || m_currentSeasonIdx >= m_seasonNumbers.size()) {
        m_episodes->setEpisodes({});
...
void SeriesDetailViewModel::refreshEpisodeWatchedState()
{
    ...
    for (int seasonNum : m_seasonNumbers) {
        ...
        newSeasonWatched.append(seasonAired > 0 && seasonWatchedCount == seasonAired
                                && !hasUpcomingEpisode);
    }
...
{
    return m_inLibrary ? i18nc("@action:button", "Remove from Library")
...
void SeriesDetailViewModel::loadAt(const QString& imdbId, int season, int episode)
{
    if (imdbId.isEmpty()) {
        return;
    }
...
void SeriesDetailViewModel::retry()
{
    ...
    auto pendingSeason =
        m_selectedEpisodeRow >= 0 ? std::optional<int>(m_selectedEpisode.season) : std::nullopt;
    auto pendingEpisode =
        m_selectedEpisodeRow >= 0 ? std::optional<int>(m_selectedEpisode.number) : std::nullopt;
    auto t = loadSeriesMetaTask(m_imdbId, pendingSeason, pendingEpisode);
    Q_UNUSED(t);
}

QCoro::Task<void> SeriesDetailViewModel::loadSeriesMetaTask(QString imdbId,
...
{
    const auto* ep = m_episodes->at(row);
    if (!m_watched || !ep || m_imdbId.isEmpty() || core::isFutureRelease(ep->released)) {
        return;
    }
    const bool watched = m_watched->isEpisodeWatched(m_imdbId, ep->season, ep->number);
    m_watched->setEpisodeWatched(m_imdbId, ep->season, ep->number, !watched);
}
...
QCoro::Task<void> SeriesDetailViewModel::loadEpisodeStreamsTask(domain::Episode ep)
{
    ...
    if (ep.released && core::isReleaseTooEarlyForStreams(ep.released)) {
    ...
    try {
        ...
        if (!indexer) {
            m_streams->setError(i18nc("@info streams empty", "No stream indexer is configured."));
            co_return;
        }
        auto streams = co_await indexer->streams(domain::MediaKind::Series, ep.streamId(m_imdbId));
        if (myEpoch != m_episodeEpoch) {
            co_return;
        }
        m_streamState->setRawStreams(std::move(streams));
        m_streamState->rebuildVisibleStreams();
    } catch (const std::exception& e) {
        if (myEpoch != m_episodeEpoch) {
            co_return;
        }
        m_streams->setError(core::describeError(e, "episode streams"));
    }
}

QCoro::Task<void> SeriesDetailViewModel::resolveByTmdbAndLoad(int tmdbId, QString title)
{
    Q_EMIT statusMessage(i18nc("@info:status", "Looking up \u201c%1\u201d\u2026", title), 0);
    ...
    try {
        ...
        if (const auto* he = core::asHttpError(e)) {
            const bool notFound =
                he->kind() == core::HttpError::Kind::HttpStatus && he->httpStatus() == 404;
            qCWarning(KINEMA_UI).nospace()
                << "TMDB external_ids lookup failed: tv/" << tmdbId << " (\"" << title
    ...
    if (imdbId.isEmpty()) {
        qCWarning(KINEMA_UI).nospace()
            << "TMDB has no IMDB id for tv/" << tmdbId << " (\"" << title << "\")";
        Q_EMIT statusMessage(i18nc("@info:status",
...
QCoro::Task<void> SeriesDetailViewModel::loadSimilarFor(QString imdbId)
{
    ...
    try {
        ...
        Q_UNUSED(core::describeError(e, "series detail/similar/recommendations"));
        m_similar->setItems({});
        setSimilarVisible(false);
        co_return;
    }
...
domain::PlaybackContext SeriesDetailViewModel::currentContext() const
{
    ...
    ctx.title = m_selectedEpisodeRow >= 0 ? m_selectedEpisodeLabel : m_title;
    ctx.poster = QUrl(m_posterUrl);
    ctx.backdrop = QUrl(m_backdropUrl);
    return ctx;
}

void SeriesDetailViewModel::setDownloadController(controllers::DownloadController* dl)
{
    m_downloads = dl;
}
...
void SeriesDetailViewModel::playNow(int row)
{
    ...
    if (s->directUrl.isEmpty() && s->infoHash.isEmpty()) {
        Q_EMIT statusMessage(i18nc("@info:status", "This stream has no playable URL or magnet."),
                             4000);
        return;
    }
    if (!m_playback) {
...
void SeriesDetailViewModel::playWithBackend(int row, int backendKind)
{
    ...
    m_playback->playWithBackend(
        *s, currentContext(), static_cast<domain::DownloadBackendKind>(backendKind));
}
...
void SeriesDetailViewModel::download(int row)
{
    ...
    Q_EMIT statusMessage(i18nc("@info:status starting a background episode download",
                               "Downloading \u201c%1\u201d\u2026",
...
void SeriesDetailViewModel::downloadWithBackend(int row, int backendKind)
{
    ...
    m_downloads->downloadWithBackend(
        *s, currentContext(), static_cast<domain::DownloadBackendKind>(backendKind));
}

void SeriesDetailViewModel::copyMagnet(int row)
{
    dispatchStreamAction(m_streams, m_streamActions, row, &services::StreamActions::copyMagnet);
}

void SeriesDetailViewModel::openMagnet(int row)
{
    dispatchStreamAction(m_streams, m_streamActions, row, &services::StreamActions::openMagnet);
}

void SeriesDetailViewModel::copyDirectUrl(int row)
{
    dispatchStreamAction(m_streams, m_streamActions, row, &services::StreamActions::copyDirectUrl);
}

void SeriesDetailViewModel::openDirectUrl(int row)
{
    dispatchStreamAction(m_streams, m_streamActions, row, &services::StreamActions::openDirectUrl);
}

void SeriesDetailViewModel::copyReleaseName(int row)
{
    dispatchStreamAction(
        m_streams, m_streamActions, row, &services::StreamActions::copyReleaseName);
}
...
void SeriesDetailViewModel::addSimilarToLibrary(int row)
{
    ...
    auto task = title_actions::addToLibraryByTmdb(
        m_tmdb, m_library, this, item->tmdbId, item->kind, item->title);
    Q_UNUSED(task);
}
...
void SeriesDetailViewModel::markSimilarWatched(int row)
{
    ...
    auto task = title_actions::markWatchedByTmdb(
        m_tmdb, m_watched, this, item->tmdbId, item->kind, item->title);
    Q_UNUSED(task);
}
...
void SeriesDetailViewModel::findSimilarStreams(int row)
{
    ...
    if (item->kind == domain::MediaKind::Series) {
        Q_EMIT findSeriesStreamsByTmdbRequested(item->tmdbId, item->title);
    } else {
        Q_EMIT findMovieStreamsByTmdbRequested(item->tmdbId, item->title);
    }
}
