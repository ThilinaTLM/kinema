 ...
 * `ShellViewModel`. NOT registered with `QML_ELEMENT` because the
...
public:
    ...
    /// Used by tests / `ShellViewModel` to feed the section list at
...
public Q_SLOTS:
    ...
    /// stubs the real navigation in `ShellViewModel` with a passive
...
Q_SIGNALS:
    ...
    /// Routed by `ShellViewModel` to `BrowseViewModel::applyPreset`
