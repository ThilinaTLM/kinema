...
DiscoverViewModel::DiscoverViewModel(api::TmdbClient* tmdb,
                                     ...
                                     QObject* parent)
    : QObject(parent), m_tmdb(tmdb), m_tokens(tokens)
{
    buildSections();
...
{
    if (m_authFailed) {
        return i18nc("@label discover placeholder", "TMDB rejected the token");
    }
    if (!m_tmdbConfigured) {
        return i18nc("@label discover placeholder", "Set up TMDB to enable Discover");
    }
    return {};
}
...
void DiscoverViewModel::buildSections()
{
    ...
    const QList<SectionSpec> specs = {
        {i18nc("@label discover row", "Trending this week"),
         wrap([](api::TmdbClient* c) { return c->trending(domain::MediaKind::Movie, true); }),
         domain::MediaKind::Movie,
         domain::DiscoverSort::Popularity},
        {i18nc("@label discover row", "Popular series"),
         wrap([](api::TmdbClient* c) { return c->popular(domain::MediaKind::Series); }),
        ...
        {i18nc("@label discover row", "Top rated movies"),
         wrap([](api::TmdbClient* c) { return c->topRated(domain::MediaKind::Movie); }),
         domain::MediaKind::Movie,
         domain::DiscoverSort::Rating},
        {i18nc("@label discover row", "Top rated series"),
         wrap([](api::TmdbClient* c) { return c->topRated(domain::MediaKind::Series); }),
         domain::MediaKind::Series,
         domain::DiscoverSort::Rating},
    };
...
QCoro::Task<void> DiscoverViewModel::loadSection(int idx)
{
    ...
    try {
        ...
        if ((core::isHttpStatus(e, 401) || core::isHttpStatus(e, 403)) && !m_authFailed) {
            setAuthFailed(true);
...
void DiscoverViewModel::browseSection(int sectionIndex)
{
    ...
    Q_EMIT browseRequested(static_cast<int>(m_browseKinds.at(sectionIndex)),
...
void DiscoverViewModel::setLibraryController(controllers::LibraryController* lib)
{
    m_library = lib;
}

void DiscoverViewModel::setWatchedController(controllers::WatchedController* watched)
{
    m_watched = watched;
}
