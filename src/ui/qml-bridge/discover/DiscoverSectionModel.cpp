...
DiscoverSectionModel::DiscoverSectionModel(QObject* parent) : QAbstractListModel(parent) { }
...
void DiscoverSectionModel::setProgressList(QList<double> progress)
{
    ...
    if (!m_items.isEmpty()) {
        Q_EMIT dataChanged(index(0), index(rowCount() - 1), {ProgressRole});
    }
}
...
void DiscoverSectionModel::setLastReleaseList(QStringList releases)
{
    ...
    if (!m_items.isEmpty()) {
        Q_EMIT dataChanged(index(0), index(rowCount() - 1), {LastReleaseRole});
    }
}
...
void DiscoverSectionModel::setEpisodeSubtitleList(QStringList subtitles)
{
    ...
    if (!m_items.isEmpty()) {
        Q_EMIT dataChanged(index(0), index(rowCount() - 1), {EpisodeSubtitleRole});
    }
}
