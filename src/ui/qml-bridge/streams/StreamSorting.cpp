...
{
    if (res == QLatin1String("2160p"))
        return 2160;
    if (res == QLatin1String("1440p"))
        return 1440;
    if (res == QLatin1String("1080p"))
        return 1080;
    if (res == QLatin1String("720p"))
        return 720;
    if (res == QLatin1String("480p"))
        return 480;
    if (res == QLatin1String("360p"))
        return 360;
    return 0;
}
...
{
    return !resolution.isEmpty() || hdrOnly || dolbyVisionOnly || multiAudioOnly;
}

QList<domain::Stream> applyUiFilters(QList<domain::Stream> rows, const UiFilters& filters)
{
    if (!filters.any()) {
        return rows;
    }
    ...
    for (auto& s : rows) {
        ...
        if (filters.hdrOnly || filters.dolbyVisionOnly || filters.multiAudioOnly) {
            ...
            if (filters.dolbyVisionOnly && t.hdr != core::stream_tokens::Hdr::DolbyVision) {
                continue;
            }
            if (filters.hdrOnly && t.hdr == core::stream_tokens::Hdr::Sdr) {
                continue;
            }
...
void sortInPlace(QList<domain::Stream>& rows, StreamsListModel::SortMode mode, bool descending)
{
    ...
    if (mode == SortMode::Smart) {
        ...
        std::stable_sort(
            rows.begin(), rows.end(), [](const domain::Stream& a, const domain::Stream& b) {
    ...
    if (descending) {
        std::stable_sort(
            rows.begin(), rows.end(), [cmp](const domain::Stream& a, const domain::Stream& b) {
                return cmp(b, a);
            });
    } else {
