...
SubtitlesViewModel::SubtitlesViewModel(controllers::SubtitleController* controller,
    ...
    , m_model(new SubtitleResultsModel(this))
{
    ...
    connect(&m_releaseDebounce, &QTimer::timeout, this, &SubtitlesViewModel::runSearch);
    ...
    if (m_controller) {
        ...
        connect(m_controller,
                ...
                this,
                &SubtitlesViewModel::onDownloadEnabledChanged);
        connect(m_controller,
                &controllers::SubtitleController::quotaChanged,
                this,
...
void SubtitlesViewModel::setMedia(const domain::PlaybackContext& ctx)
{
    ...
    if (m_controller && m_controller->downloadEnabled() && m_context.key.isValid()) {
        runSearch();
    }
}
...
void SubtitlesViewModel::runSearch()
{
    ...
    m_controller->runQuery(m_context.key, m_languages, m_hi, m_fpo, m_release.trimmed());
}
...
{
    if (!m_controller || m_selectedRow < 0 || m_selectedRow >= m_model->count()) {
        return;
    }
...
{
    Q_EMIT downloadCompleted(m_context.key, fileId, localPath, language, languageName);
...
void SubtitlesViewModel::onDownloadFailed(const QString&, const QString&)
{
    updateStatusLine();
    updatePrimarySemantics();
}
...
void SubtitlesViewModel::updateState()
{
    ...
    if (!m_controller) {
        ...
        next = m_model->count() == 0 && m_release.isEmpty() ? QStringLiteral("idle")
...
void SubtitlesViewModel::updateStatusLine()
{
    ...
    if (m_controller->isSearching()) {
        parts << i18nc("@info:status subtitles search in progress", "Searching…");
    } else {
        const int n = m_controller->hits().size();
        parts << i18ncp("@info:status subtitles result count", "%1 result", "%1 results", n);
    }
    const int remaining = m_controller->dailyDownloadsRemaining();
    if (remaining >= 0) {
        parts << i18nc("@info:status OpenSubtitles daily download quota",
                       "Daily quota: %1 remaining",
                       remaining);
    }
...
void SubtitlesViewModel::updatePrimarySemantics()
{
    ...
    QString icon;
    bool enabled = false;
    const bool gate = m_controller && m_controller->downloadEnabled() && m_context.key.isValid();

    if (!gate || m_selectedRow < 0 || m_selectedRow >= m_model->count()) {
        text = i18nc("@action:button subtitle row primary action default label", "Download");
        ...
        const bool isCached = m_controller->cachedFileIds().contains(hit.fileId);
        if (isCached) {
            text = i18nc("@action:button subtitle row action for cached entry", "Use");
            icon = QStringLiteral("circle-check");
        } else {
            text = i18nc("@action:button subtitle row action for fresh download", "Download");
            icon = QStringLiteral("download");
        }
        enabled = true;
    }
...
QString SubtitlesViewModel::formatContextTitle(const domain::PlaybackContext& ctx) const
{
    QString base = ctx.title.isEmpty() ? i18nc("@title placeholder when media has no display title",
                                               "Selected media")
                                       : ctx.title;
    if (ctx.key.kind == domain::MediaKind::Series && ctx.key.season.has_value()
        ...
        const auto series = ctx.seriesTitle.isEmpty() ? base : ctx.seriesTitle;
        ...
        return i18nc(
            "@info subtitles page header for an episode without title", "%1 — %2", series, tag);
    }
    return base;
}
