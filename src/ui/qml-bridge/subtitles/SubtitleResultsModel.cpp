...
QIcon flagsIcon(bool hi, bool fpo)
{
    ...
    const QIcon hiIcon = hi ? QIcon::fromTheme(QStringLiteral("audio-headset-symbolic"),
                                               QIcon::fromTheme(QStringLiteral("audio-headset")))
                            : QIcon{};
    const QIcon fpoIcon = fpo ? QIcon::fromTheme(QStringLiteral("flag-symbolic"),
...
QString flagsTooltip(bool hi, bool fpo)
{
    QStringList parts;
    if (hi) {
        parts << i18nc("@info:tooltip subtitle flag", "Hearing impaired");
    }
    if (fpo) {
        parts << i18nc("@info:tooltip subtitle flag", "Foreign parts only");
    }
...
SubtitleResultsModel::SubtitleResultsModel(QObject* parent) : QAbstractListModel(parent) { }
...
QVariant SubtitleResultsModel::data(const QModelIndex& index, int role) const
{
    ...
    if (role >= FileIdRole && role <= ActiveRole) {
        ...
        case FpsRole:
            return h.fps;
        case HearingImpairedRole:
            return h.hearingImpaired;
        case ForeignPartsOnlyRole:
            return h.foreignPartsOnly;
        case MoviehashMatchRole:
            return h.moviehashMatch;
        case CachedRole:
            return m_cached.contains(h.fileId);
        case ActiveRole:
            return m_active.contains(h.fileId);
        default:
            return {};
        }
    }
    ...
    case Qt::DisplayRole:
        ...
        case DownloadsColumn:
            if (h.downloadCount <= 0)
                return QString{};
            return i18ncp("@info downloads count", "%1 download", "%1 downloads", h.downloadCount);
        case RatingColumn:
    ...
    case Qt::ToolTipRole:
        ...
        case ReleaseColumn: {
            ...
            if (!h.fileName.isEmpty())
                lines << h.fileName;
            if (m_active.contains(h.fileId)) {
                lines << i18nc("@info:tooltip", "Currently loaded in the player");
            } else if (m_cached.contains(h.fileId)) {
                lines << i18nc("@info:tooltip", "Already downloaded");
            }
            if (h.moviehashMatch) {
                lines << i18nc("@info:tooltip", "Matches the file's moviehash");
            }
...
QVariant SubtitleResultsModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    case DownloadsColumn:
        return i18nc("@title:column subtitle download count", "Downloads");
    case RatingColumn:
