...
TmdbSettingsViewModel::TmdbSettingsViewModel(core::HttpClient* http,
                                             core::TokenStore* tokens,
                                             QObject* parent)
    : QObject(parent), m_http(http), m_tokens(tokens)
{ }

bool TmdbSettingsViewModel::hasCompiledDefault() const
{
    return core::kTmdbCompiledDefaultToken != nullptr && core::kTmdbCompiledDefaultToken[0] != '\0';
}
...
QCoro::Task<void> TmdbSettingsViewModel::loadTask()
{
    ...
    try {
        const auto existing =
            co_await m_tokens->read(QString::fromLatin1(core::TokenStore::kTmdbKey));
        if (!existing.isEmpty()) {
            setToken(existing);
            if (!m_userTokenSaved) {
            ...
            setStatus(i18nc("@info tmdb settings status", "Using your token."), kStatusPositive);
        } else if (hasCompiledDefault()) {
            setStatus(i18nc("@info tmdb settings status", "Using the bundled token."), kStatusInfo);
        } else {
            setStatus(
                i18nc("@info tmdb settings status", "No token configured. Discover is disabled."),
                kStatusError);
        }
    } catch (const core::TokenStoreError& e) {
        setStatus(e.message(), kStatusError);
    } catch (const std::exception& e) {
        setStatus(core::describeError(e, "tmdb settings/load"), kStatusError);
    }
    setBusy(false);
}
...
QCoro::Task<void> TmdbSettingsViewModel::testTask()
{
    ...
    if (token.isEmpty()) {
        if (m_userTokenSaved) {
            try {
                token = co_await m_tokens->read(QString::fromLatin1(core::TokenStore::kTmdbKey));
            } catch (const std::exception& e) {
                setStatus(core::describeError(e, "tmdb settings/read saved token"), kStatusError);
                co_return;
            }
        }
    ...
    setBusy(true);
    setStatus(i18nc("@info tmdb settings status, in progress", "Testing TMDB credentials…"),
              kStatusInfo);
    api::TmdbClient client(m_http);
    client.setToken(token);
    try {
        co_await client.testAuth();
        setStatus(i18nc("@info tmdb settings status", "Connected to TMDB."), kStatusPositive);
    } catch (const std::exception& e) {
        if (const auto* he = core::asHttpError(e);
            ...
            setStatus(core::describeError(e, "tmdb settings/test auth"), kStatusError);
        }
    }
    setBusy(false);
}
...
QCoro::Task<void> TmdbSettingsViewModel::saveTask()
{
    ...
    setBusy(true);
    try {
        co_await m_tokens->write(QString::fromLatin1(core::TokenStore::kTmdbKey), token);
        if (!m_userTokenSaved) {
            m_userTokenSaved = true;
            Q_EMIT userTokenSavedChanged();
        }
        Q_EMIT tokenChanged(token);
        setStatus(i18nc("@info tmdb settings status", "Token saved to keyring. Using your token."),
                  kStatusPositive);
    } catch (const core::TokenStoreError& e) {
        setStatus(e.message(), kStatusError);
    } catch (const std::exception& e) {
        setStatus(core::describeError(e, "tmdb settings/save"), kStatusError);
    }
    setBusy(false);
}
...
QCoro::Task<void> TmdbSettingsViewModel::removeTask()
{
    setBusy(true);
    try {
        co_await m_tokens->remove(QString::fromLatin1(core::TokenStore::kTmdbKey));
        m_token.clear();
        Q_EMIT tokenInputChanged();
        if (m_userTokenSaved) {
        ...
        setStatus(hasCompiledDefault()
                      ? i18nc("@info tmdb settings status",
                              ...
                              "token.")
                      : i18nc("@info tmdb settings status", "Token removed. No token configured."),
                  kStatusInfo);
    } catch (const core::TokenStoreError& e) {
        setStatus(e.message(), kStatusError);
    } catch (const std::exception& e) {
        setStatus(core::describeError(e, "tmdb settings/remove"), kStatusError);
    }
    setBusy(false);
}
