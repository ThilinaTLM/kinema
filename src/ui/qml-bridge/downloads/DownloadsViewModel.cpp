...
DownloadsViewModel::DownloadsViewModel(controllers::DownloadController& controller,
    ...
    , m_items(new DownloadsListModel(this))
{
    ...
    connect(&m_controller,
            &controllers::DownloadController::changed,
            this,
    ...
    connect(&m_controller,
            &controllers::DownloadController::itemChanged,
            this,
            &DownloadsViewModel::onItemChanged);
    refresh();
}
...
void DownloadsViewModel::recomputeAggregatesFromModel()
{
    ...
    if (prevActive != m_activeCount || prevPaused != m_pausedCount
        || prevCompleted != m_completedCount || prevFailed != m_failedCount
        || prevTotal != m_totalCount || prevRate != m_totalRateBps) {
        Q_EMIT countsChanged();
    }
}
...
void DownloadsViewModel::rebuildCountsFrom(const QList<domain::DownloadItem>& rows,
...
QString DownloadsViewModel::totalDownloadRateText() const
{
    ...
    return i18nc("@label aggregate download rate per second",
                 "%1/s",
                 KFormat().formatByteSize(m_totalRateBps));
}
...
{
    if (!m_playback) {
        qCWarning(KINEMA_UI) << "playDownload: PlaybackSessionManager not wired";
        return;
    }
    try {
        const auto rows = m_controller.items();
        const auto it = std::find_if(rows.begin(), rows.end(), [&](const domain::DownloadItem& r) {
            return r.assetId == assetId;
        });
        if (it == rows.end()) {
            qCWarning(KINEMA_UI) << "playDownload: no row for assetId" << assetId;
            return;
        }
        ...
        qCWarning(KINEMA_UI) << "playDownload failed:" << core::describeError(e, "play download");
    }
}
...
void DownloadsViewModel::openLocalDir(const QString& assetId)
{
    ...
    if (!row) {
        qCWarning(KINEMA_UI) << "openLocalDir: no row for assetId" << assetId;
        return;
    }

    const auto loc = core::locateAsset(*row);
    if (loc.dir.isEmpty()) {
        qCWarning(KINEMA_UI) << "openLocalDir: unresolved location for assetId" << assetId;
        Q_EMIT statusMessage(i18nc("@info:status", "No local folder for this download yet"), 5000);
        return;
    }
    if (!loc.exists) {
        qCWarning(KINEMA_UI).nospace().noquote()
            << "openLocalDir: dir missing assetId=\"" << assetId << "\" dir=\"" << loc.dir << "\"";
        Q_EMIT statusMessage(i18nc("@info:status", "Folder no longer exists: %1", loc.dir), 6000);
        return;
    }
    ...
    qCDebug(KINEMA_UI).nospace().noquote() << "openLocalDir: opening \"" << loc.dir << "\"";
    core::io::openExternal(
        QUrl::fromLocalFile(loc.dir),
        this,
        [this, dir = loc.dir](const core::io::OpenExternalResult& r) {
            ...
            qCWarning(KINEMA_UI).nospace().noquote()
                << "openLocalDir failed dir=\"" << dir << "\" error=\"" << r.errorString << "\"";
            Q_EMIT statusMessage(i18nc("@info:status", "Could not open folder: %1", r.errorString),
                                 6000);
        });
}
