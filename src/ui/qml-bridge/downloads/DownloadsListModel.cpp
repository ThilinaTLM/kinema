...
QString stateText(domain::DownloadState s, domain::DownloadMode mode, bool hasPlayer, bool complete)
{
    ...
    case S::Active:
        if (mode == M::OnDemand) {
            return hasPlayer ? i18nc("@label download state", "Streaming")
                             : i18nc("@label download state", "Caching");
        }
        return hasPlayer ? i18nc("@label download state", "Downloading + Playing")
...
QString rateText(qint64 bps)
{
    ...
    return i18nc("@label download rate per second", "%1/s", KFormat().formatByteSize(bps));
}
...
QString etaText(int seconds)
{
    ...
    return KFormat().formatDuration(static_cast<quint64>(seconds) * 1000ULL,
...
DownloadsListModel::DownloadsListModel(QObject* parent) : QAbstractListModel(parent) { }
...
QVariant DownloadsListModel::data(const QModelIndex& index, int role) const
{
    ...
    case ExpectedSizeBytesRole:
        return it.expectedSizeBytes ? QVariant(*it.expectedSizeBytes) : QVariant();
    case SizeTextRole:
        return sizeText(it);
...
void DownloadsListModel::updateRow(const QString& assetId, const domain::DownloadItem& item)
{
...
void DownloadsListModel::updateLiveStatsFor(const QString& assetId, const LiveRow& live)
{
    const int row = rowForAssetId(assetId);
    if (row < 0) {
        return;
