...
AudioTracksModel::AudioTracksModel(QObject* parent)
    : QAbstractListModel(parent)
{
}

int AudioTracksModel::rowCount(const QModelIndex& parent) const
{
    if (parent.isValid()) {
        return 0;
    }
...
QVariant AudioTracksModel::data(const QModelIndex& index, int role) const
{
    ...
    case ChannelsRole:  return e.channelCount;
...
QHash<int, QByteArray> AudioTracksModel::roleNames() const
{
    return {
        ...
        { ChannelsRole, QByteArrayLiteral("channels") },
        { SelectedRole, QByteArrayLiteral("selected") },
    };
}

void AudioTracksModel::reset(const core::tracks::TrackList& tracks)
{
    beginResetModel();
