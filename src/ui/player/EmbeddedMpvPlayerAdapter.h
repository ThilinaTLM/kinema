 ...
 * is intentionally minimal: `PlaybackSession` is the real owner of the lifecycle. The adapter just
...
class EmbeddedMpvPlayerAdapter
    : public QObject
    , public playback::ports::EmbeddedPlayerPort
{
    Q_OBJECT
public:
    EmbeddedMpvPlayerAdapter(playback::events::PlaybackEventStream& eventStream,
    ...
    void setActiveSession(playback::PlaybackSessionId sessionId,
                          const domain::PlaybackContext& ctx) override;
    void setVisibilityHandler(VisibilityHandler handler) override;
    void setStatusHandler(StatusHandler handler) override;

    playback::PlaybackSessionId activeSessionId() const noexcept { return m_sessionId; }
    ...
    playback::session::PlayerLoadWatchdog& loadWatchdog() noexcept { return m_loadWatchdog; }
    ...
    bool attachSubtitleFile(const QString& localPath, const QString& language) override;
    playback::ports::PlayerSnapshot snapshot() const override;
...
private:
    ...
    /// `playback::PlaybackEndReason`. Returns nullopt when the reason should
    /// be filtered (intermediate "stop" caused by loadfile
    /// replacing the previous file).
    std::optional<playback::PlaybackEndReason> classifyEndReason(const QString& reason);

    playback::events::PlaybackEventStream& m_eventStream;
    const config::PlayerSettings& m_settings;
    QPointer<ui::player::PlayerWindow> m_window;
    playback::session::PlayerLoadWatchdog m_loadWatchdog{this};

    playback::PlaybackSessionId m_sessionId;
    ...
    bool m_paused = false;
    VisibilityHandler m_visibilityHandler;
