    ...
    , m_mainWindow(mainWindow)
{
    ...
    if (!m_available) {
        qCInfo(KINEMA_APP) << "no system tray host available;"
...
QIcon TrayController::renderSymbolicIcon() const
{
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        qCWarning(KINEMA_APP) << "tray: symbolic SVG resource missing;"
    ...
    if (!renderer.isValid()) {
        qCWarning(KINEMA_APP) << "tray: symbolic SVG failed to parse";
