...
core::HttpError missingCredentials()
{
    return core::HttpError(
        core::HttpError::Kind::HttpStatus, 401, i18n("OpenSubtitles credentials not configured."));
}
...
QCoro::Task<QList<SubtitleHit>> OpenSubtitlesClient::search(SubtitleSearchQuery q)
{
    if (!hasCredentials()) {
    ...
    if (!q.key.isValid()) {
        throw core::HttpError(
            core::HttpError::Kind::HttpStatus, 400, i18n("Subtitle search requires an IMDb id."));
    }
    ...
    if (q.key.season.has_value()) {
        query.addQueryItem(QStringLiteral("season_number"), QString::number(*q.key.season));
    }
    if (q.key.episode.has_value()) {
        query.addQueryItem(QStringLiteral("episode_number"), QString::number(*q.key.episode));
    }
    if (!q.languages.isEmpty()) {
        query.addQueryItem(QStringLiteral("languages"), q.languages.join(QLatin1Char(',')));
    }
    if (!q.hearingImpaired.isEmpty() && q.hearingImpaired != QLatin1String("off")) {
        query.addQueryItem(QStringLiteral("hearing_impaired"), q.hearingImpaired);
    }
    if (!q.foreignPartsOnly.isEmpty() && q.foreignPartsOnly != QLatin1String("off")) {
        query.addQueryItem(QStringLiteral("foreign_parts_only"), q.foreignPartsOnly);
    }
...
QCoro::Task<SubtitleDownloadTicket> OpenSubtitlesClient::requestDownload(QString fileId)
{
    if (!hasCredentials()) {
        throw missingCredentials();
    }
    if (fileId.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::HttpStatus, 400, i18n("Missing subtitle file id."));
    }
    ...
    const auto post = [&]() -> QCoro::Task<QJsonDocument> {
        auto req = baseRequest(buildUrl(QStringLiteral("/download")),
                               /*authed=*/true);
        co_return co_await m_http->postJsonForJson(std::move(req), bytes);
    };
