...
#include "api/common/RequestUtil.h"
...
core::HttpError notConfigured()
{
    return core::HttpError(
        core::HttpError::Kind::HttpStatus, 401, i18n("No TMDB token configured."));
}

QString kindMovieOrTv(MediaKind kind)
{
    return kind == MediaKind::Movie ? QStringLiteral("movie") : QStringLiteral("tv");
}
...
{
    requireToken();
    const auto window = weekly ? QStringLiteral("week") : QStringLiteral("day");
    const auto url = buildUrl(QStringLiteral("/trending/%1/%2").arg(kindMovieOrTv(kind), window));
...
{
    requireToken();
    const auto url = buildUrl(QStringLiteral("/%1/popular").arg(kindMovieOrTv(kind)));
    co_return tmdb::parseList(co_await fetch(url), kind);
}
...
{
    requireToken();
    const auto url = buildUrl(QStringLiteral("/%1/top_rated").arg(kindMovieOrTv(kind)));
    co_return tmdb::parseList(co_await fetch(url), kind);
}
...
QCoro::Task<QList<DiscoverItem>> TmdbClient::nowPlayingMovies()
{
    requireToken();
    const auto url =
        buildUrl(QStringLiteral("/movie/now_playing"), {{QStringLiteral("region"), m_region}});
...
QCoro::Task<QString> TmdbClient::imdbIdForTmdbMovie(int tmdbId)
{
    requireToken();
    const auto url =
        buildUrl(QStringLiteral("/movie/%1").arg(tmdbId),
                 {{QStringLiteral("append_to_response"), QStringLiteral("external_ids")}});
...
{
    requireToken();
    const auto url = buildUrl(QStringLiteral("/tv/%1/external_ids").arg(tmdbId));
    co_return tmdb::parseSeriesExternalIds(co_await fetch(url));
}

QCoro::Task<std::pair<int, MediaKind>> TmdbClient::findByImdb(QString imdbId,
...
QCoro::Task<QList<DiscoverItem>> TmdbClient::recommendations(MediaKind kind, int tmdbId)
{
    ...
    const auto url = buildUrl(
        QStringLiteral("/%1/%2/recommendations").arg(kindMovieOrTv(kind), QString::number(tmdbId)));
    co_return tmdb::parseList(co_await fetch(url), kind);
}
...
QCoro::Task<QList<DiscoverItem>> TmdbClient::similar(MediaKind kind, int tmdbId)
{
    requireToken();
    const auto url = buildUrl(
        QStringLiteral("/%1/%2/similar").arg(kindMovieOrTv(kind), QString::number(tmdbId)));
    co_return tmdb::parseList(co_await fetch(url), kind);
}
...
{
    requireToken();
    const auto url = buildUrl(tmdb::discoverPath(q.kind), tmdb::discoverQueryToQuery(q));
    co_return tmdb::parsePagedList(co_await fetch(url), q.kind);
}

QCoro::Task<DiscoverPageResult> TmdbClient::search(MediaKind kind, QString query, int page)
{
    requireToken();
    const auto url = buildUrl(QStringLiteral("/search/%1").arg(kindMovieOrTv(kind)),
...
QCoro::Task<QList<TmdbGenre>> TmdbClient::genreList(MediaKind kind)
{
    ...
    requireToken();
    const auto url = buildUrl(QStringLiteral("/genre/%1/list").arg(kindMovieOrTv(kind)));
