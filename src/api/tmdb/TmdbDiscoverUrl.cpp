...
{
    return kind == MediaKind::Movie ? QStringLiteral("/discover/movie")
...
QString discoverSortValue(MediaKind kind, DiscoverSort sort)
{
    ...
    case DiscoverSort::ReleaseDate:
        return kind == MediaKind::Movie ? QStringLiteral("primary_release_date.desc")
    ...
    case DiscoverSort::TitleAsc:
        return kind == MediaKind::Movie ? QStringLiteral("original_title.asc")
...
QUrlQuery discoverQueryToQuery(const DiscoverQuery& q)
{
    ...
    out.addQueryItem(QStringLiteral("include_adult"), QStringLiteral("false"));
    if (q.kind == MediaKind::Movie) {
        out.addQueryItem(QStringLiteral("include_video"), QStringLiteral("false"));
    }
    out.addQueryItem(QStringLiteral("sort_by"), discoverSortValue(q.kind, q.sort));
    ...
    if (!q.withGenreIds.isEmpty()) {
        ...
        out.addQueryItem(QStringLiteral("with_genres"), ids.join(QLatin1Char(',')));
    }

    const auto gteKey = q.kind == MediaKind::Movie ? QStringLiteral("primary_release_date.gte")
                                                   : QStringLiteral("first_air_date.gte");
    const auto lteKey = q.kind == MediaKind::Movie ? QStringLiteral("primary_release_date.lte")
    ...
    if (effectiveVoteCount > 0) {
        out.addQueryItem(QStringLiteral("vote_count.gte"), QString::number(effectiveVoteCount));
    }

    if (q.page > 1) {
        out.addQueryItem(QStringLiteral("page"), QString::number(q.page));
    }
