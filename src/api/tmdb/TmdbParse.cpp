...
{
    const auto obj = requireObject(doc, i18n("TMDB list response was not a JSON object."));
...
{
    const auto obj = requireObject(doc, i18n("TMDB paged list response was not a JSON object."));
...
{
    const auto obj = requireObject(doc, i18n("TMDB genre list response was not a JSON object."));
...
{
    return extractImdbId(requireObject(doc, i18n("TMDB movie response was not a JSON object.")));
}

QString parseSeriesExternalIds(const QJsonDocument& doc)
{
    return extractImdbId(requireObject(doc, i18n("TMDB series response was not a JSON object.")));
}

std::pair<int, MediaKind> parseFindResult(const QJsonDocument& doc, MediaKind preferredKind)
{
    const auto obj = requireObject(doc, i18n("TMDB /find response was not a JSON object."));
