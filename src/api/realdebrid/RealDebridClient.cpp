...
void requireToken(const QString& token)
{
    if (token.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::HttpStatus, 401, i18n("No Real-Debrid token set."));
    }
}
...
QCoro::Task<RdAddMagnetResult> RealDebridClient::addMagnet(QString magnet)
{
    ...
    if (magnet.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("Real-Debrid addMagnet: empty magnet URI."));
    }
...
QCoro::Task<RdTorrentInfo> RealDebridClient::torrentInfo(QString rdTorrentId)
{
    ...
    if (rdTorrentId.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("Real-Debrid torrentInfo: empty torrent id."));
    }

    const QUrl url = appendPath(m_baseUrl, QStringLiteral("/torrents/info/") + rdTorrentId);
    QNetworkRequest req(bearerGet(url, bearer(m_token)));
...
QCoro::Task<void> RealDebridClient::selectFiles(QString rdTorrentId, QList<int> fileIds)
{
    requireToken(m_token);
    if (rdTorrentId.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("Real-Debrid selectFiles: empty torrent id."));
    }

    const QUrl url = appendPath(m_baseUrl, QStringLiteral("/torrents/selectFiles/") + rdTorrentId);
    QNetworkRequest req(bearerFormPost(url, bearer(m_token)));
...
QCoro::Task<RdUnrestrictedLink> RealDebridClient::unrestrictLink(QUrl link)
{
    ...
    if (link.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("Real-Debrid unrestrict: empty link."));
    }
...
QCoro::Task<void> RealDebridClient::deleteTorrent(QString rdTorrentId)
{
    ...
    const QUrl url = appendPath(m_baseUrl, QStringLiteral("/torrents/delete/") + rdTorrentId);
    QNetworkRequest req(bearerGet(url, bearer(m_token)));
