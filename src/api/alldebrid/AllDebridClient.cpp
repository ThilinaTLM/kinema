...
void requireApiKey(const QString& key)
{
    if (key.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::HttpStatus, 401, i18n("No AllDebrid API key set."));
    }
}
...
AllDebridClient::AllDebridClient(core::HttpClient* http, QObject* parent)
    : QObject(parent), m_http(http), m_baseUrl(QStringLiteral("https://api.alldebrid.com"))
{ }
...
QCoro::Task<AdAddMagnetResult> AllDebridClient::uploadMagnet(QString magnetUri)
{
    ...
    if (magnetUri.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("AllDebrid magnet/upload: empty magnet URI."));
    }

    auto req = bearerFormPost(appendPath(m_baseUrl, QStringLiteral("/v4/magnet/upload")),
                              bearer(m_apiKey));
...
QCoro::Task<AdMagnetStatus> AllDebridClient::magnetStatus(qint64 magnetId)
{
    ...
    if (magnetId <= 0) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("AllDebrid magnet/status: invalid magnet id."));
    }

    auto req = bearerFormPost(appendPath(m_baseUrl, QStringLiteral("/v4.1/magnet/status")),
                              bearer(m_apiKey));
    const auto body = urlEncode({
...
QCoro::Task<QList<AdMagnetFile>> AllDebridClient::magnetFiles(qint64 magnetId)
{
    ...
    if (magnetId <= 0) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("AllDebrid magnet/files: invalid magnet id."));
    }

    auto req =
        bearerFormPost(appendPath(m_baseUrl, QStringLiteral("/v4/magnet/files")), bearer(m_apiKey));
    const auto body = urlEncode({
        {QStringLiteral("id[]"), QString::number(magnetId)},
    });
    const auto doc = co_await m_http->postJsonForJson(std::move(req), body);
...
QCoro::Task<AdUnlockedLink> AllDebridClient::unlockLink(QUrl link)
{
    ...
    if (link.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("AllDebrid link/unlock: empty link."));
    }

    auto req =
        bearerFormPost(appendPath(m_baseUrl, QStringLiteral("/v4/link/unlock")), bearer(m_apiKey));
    const auto body = urlEncode({
...
QCoro::Task<AdUnlockedLink> AllDebridClient::pollDelayed(qint64 delayedId)
{
    int waitedMs = 0;
    while (true) {
        auto req = bearerFormPost(appendPath(m_baseUrl, QStringLiteral("/v4/link/delayed")),
                                  bearer(m_apiKey));
        const auto body = urlEncode({
...
QCoro::Task<void> AllDebridClient::deleteMagnet(qint64 magnetId)
{
    ...
    auto req = bearerFormPost(appendPath(m_baseUrl, QStringLiteral("/v4/magnet/delete")),
                              bearer(m_apiKey));
    const auto body = urlEncode({
        {QStringLiteral("id"), QString::number(magnetId)},
    });
