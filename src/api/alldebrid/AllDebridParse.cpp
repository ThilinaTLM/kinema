...
[[noreturn]] void
throwApiError(const QString& code, const QString& message, const char* endpointLabel)
{
    int status = 400;
    ...
    const QString display = code.isEmpty()
                                ...
                                : i18n("AllDebrid %1 failed: %2 \u2014 %3",
                                       ...
                                       code,
                                       message.isEmpty() ? i18n("unknown error") : message);
    throw core::HttpError(core::HttpError::Kind::HttpStatus, status, display);
}
...
void flattenFilesTree(const QJsonArray& nodes, const QString& prefix, QList<AdMagnetFile>& out)
{
    for (const auto& v : nodes) {
        ...
        if (obj.contains(QStringLiteral("e"))) {
            const auto subPrefix = prefix.isEmpty() ? name : prefix + QLatin1Char('/') + name;
            flattenFilesTree(obj.value(QStringLiteral("e")).toArray(), subPrefix, out);
            continue;
        }
        AdMagnetFile f;
        f.path = prefix.isEmpty() ? name : prefix + QLatin1Char('/') + name;
...
QJsonObject unwrapEnvelope(const QJsonDocument& doc, const char* endpointLabel)
{
    if (!doc.isObject()) {
...
AdMagnetStatus parseMagnetStatus(const QJsonDocument& doc)
{
    ...
    if (row.isEmpty()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("AllDebrid magnet/status response is empty."));
    }
    ...
    s.statusCode = static_cast<int>(jsonInt64(row.value(QStringLiteral("statusCode"))));
    ...
    s.seeders = static_cast<int>(jsonInt64(row.value(QStringLiteral("seeders"))));
    return s;
}
...
AdUnlockedLink parseDelayed(const QJsonDocument& doc)
{
    ...
    const int status = static_cast<int>(jsonInt64(data.value(QStringLiteral("status"))));
    if (status == 3) {
