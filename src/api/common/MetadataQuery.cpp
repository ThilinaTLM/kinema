...
std::optional<QString> extractImdbTitleId(QStringView input)
{
    ...
    static const QRegularExpression bareId(QStringLiteral("^tt\\d{5,}$"),
    ...
    if (host != QStringLiteral("imdb.com") && !host.endsWith(QStringLiteral(".imdb.com"))) {
        return std::nullopt;
    }

    static const QRegularExpression titlePath(QStringLiteral("(?:^|/)title/(tt\\d{5,})(?:/|$)"),
                                              QRegularExpression::CaseInsensitiveOption);
