...
IndexerSelector::IndexerSelector(config::IndexerSettings& settings, QObject* parent)
    : QObject(parent), m_settings(settings)
{
    connect(&m_settings,
            &config::IndexerSettings::activeIndexerChanged,
            this,
            [this](domain::IndexerKind k) {
                qCDebug(KINEMA_API)
                    << "IndexerSelector: active indexer ->" << indexerKindToString(k);
