...
PeerflixIndexer::PeerflixIndexer(core::HttpClient* http,
                                 ...
                                 QObject* parent)
    : domain::Indexer(parent), m_http(http), m_settings(settings), m_creds(creds)
{ }

PeerflixIndexer::~PeerflixIndexer() = default;

QCoro::Task<QList<Stream>> PeerflixIndexer::streams(MediaKind kind, QString streamId)
{
    const auto kindStr = mediaKindToPath(kind);

    const auto creds = m_creds ? m_creds->active() : domain::ActiveDebrid{};
    const bool useDebrid = creds.provider != domain::DebridProvider::None && !creds.token.isEmpty();

    QUrl url(useDebrid ? m_settings.addonBaseUrl() : m_settings.baseUrl());
    if (useDebrid) {
        ...
        QStringList parts;
        parts.reserve(2);
        parts.append(QStringLiteral("debridoptions=torrentlinks,nocatalog"));
        parts.append(domain::providerToUrlToken(creds.provider) + QLatin1Char('=') + creds.token);
    ...
    qCInfo(KINEMA_API) << "Peerflix:" << kindStr << streamId << "\u2192" << streams.size()
                       << "streams in" << t.elapsed() << "ms";
    co_return streams;
}
