...
TorrentioIndexer::TorrentioIndexer(core::HttpClient* http,
                                   ...
                                   QObject* parent)
    : domain::Indexer(parent), m_http(http), m_settings(settings), m_filter(filter), m_creds(creds)
{ }

TorrentioIndexer::~TorrentioIndexer() = default;

QCoro::Task<QList<Stream>> TorrentioIndexer::streams(MediaKind kind, QString streamId)
{
    ...
    qCInfo(KINEMA_API) << "Torrentio:" << mediaKindToPath(kind) << streamId << "\u2192"
