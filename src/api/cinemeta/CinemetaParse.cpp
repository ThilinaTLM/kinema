...
MetaDetail parseMeta(const QJsonDocument& doc, MediaKind kind)
{
    if (!doc.isObject()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("Cinemeta meta response was not a JSON object."));
    }
    const auto metaV = doc.object().value(QStringLiteral("meta"));
    if (!metaV.isObject()) {
        throw core::HttpError(
            core::HttpError::Kind::Json, 0, i18n("Cinemeta meta response missing 'meta' object."));
    }
...
Episode episodeFromObject(const QJsonObject& obj)
{
    ...
    e.description = ov.isEmpty() ? obj.value(QStringLiteral("description")).toString() : ov;
