...
WatchedStore::overridesForImdb(const QString& imdbId) const
{
    ...
    if (!q.exec()) {
        qCWarning(KINEMA) << "WatchedStore: overridesForImdb failed:"
                          << q.lastError().text();
        return out;
    }
    while (q.next()) {
...
void WatchedStore::setOverride(const api::PlaybackKey& key, Override state)
{
    if (!m_db.isOpen() || !key.isValid()) {
        return;
    }
    ...
    if (!q.exec()) {
        qCWarning(KINEMA) << "WatchedStore: setOverride failed:"
                          << q.lastError().text();
        return;
    }
    scheduleChanged();
...
void WatchedStore::clearOverride(const api::PlaybackKey& key)
{
    if (!m_db.isOpen() || !key.isValid()) {
        return;
    }
    ...
    q.addBindValue(key.storageKey());
    if (!q.exec()) {
        qCWarning(KINEMA) << "WatchedStore: clearOverride failed:"
                          << q.lastError().text();
        return;
    }
...
void WatchedStore::clearAllForImdb(const QString& imdbId)
{
    ...
    q.addBindValue(imdbId);
    if (!q.exec()) {
        qCWarning(KINEMA) << "WatchedStore: clearAllForImdb failed:"
                          << q.lastError().text();
        return;
    }
    if (q.numRowsAffected() > 0) {
