...
domain::MediaKind mediaKindFromDb(const QString& s)
{
...
domain::LibraryTitle hydrateTitle(const QSqlQuery& q)
{
...
domain::LibraryEpisode hydrateEpisode(const QSqlQuery& q)
{
...
std::optional<domain::LibraryTitle> LibraryStore::find(
...
bool LibraryStore::contains(domain::MediaKind kind,
    const QString& imdbId) const
{
    return find(kind, imdbId).has_value();
}

QList<domain::LibraryTitle> LibraryStore::titles() const
{
    QList<domain::LibraryTitle> out;
    if (!m_db.isOpen()) {
        return out;
    }
    auto q = m_db.query();
    if (!q.exec(QStringLiteral("SELECT ") + QString::fromLatin1(kTitleColumns)
        ...
        qCWarning(KINEMA_DB) << "LibraryStore: titles failed:"
                          << q.lastError().text();
        return out;
    }
    while (q.next()) {
...
QList<domain::LibraryEpisode> LibraryStore::episodesForSeries(
    const QString& imdbId) const
{
    QList<domain::LibraryEpisode> out;
    if (!m_db.isOpen() || imdbId.isEmpty()) {
        return out;
    }
    auto q = m_db.query();
    ...
    q.addBindValue(imdbId);
    if (!q.exec()) {
        qCWarning(KINEMA_DB) << "LibraryStore: episodesForSeries failed:"
                          << q.lastError().text();
        return out;
    }
    while (q.next()) {
...
std::optional<domain::LibraryEpisode> LibraryStore::episode(
...
void LibraryStore::upsertTitle(const domain::LibraryTitle& title)
{
    ...
    if (!q.exec()) {
        qCWarning(KINEMA_DB) << "LibraryStore: upsertTitle failed:"
                          << q.lastError().text();
        return;
    }
    scheduleChanged();
...
void LibraryStore::upsertEpisodes(const QString& seriesImdbId,
    const QList<domain::LibraryEpisode>& episodes)
{
    ...
    if (!db.transaction()) {
        qCWarning(KINEMA_DB) << "LibraryStore: episode transaction failed:"
                          << db.lastError().text();
        return;
    }
    ...
    for (const auto& in : episodes) {
        ...
        if (!q.exec()) {
            qCWarning(KINEMA_DB) << "LibraryStore: upsertEpisodes failed:"
                              << q.lastError().text();
            db.rollback();
            return;
        }
    ...
    if (!db.commit()) {
        qCWarning(KINEMA_DB) << "LibraryStore: episode commit failed:"
                          << db.lastError().text();
        db.rollback();
        return;
    }
...
void LibraryStore::remove(domain::MediaKind kind, const QString& imdbId)
{
    if (!m_db.isOpen() || imdbId.isEmpty()) {
        return;
    }
    auto db = QSqlDatabase::database(m_db.connectionName());
    if (!db.transaction()) {
        qCWarning(KINEMA_DB) << "LibraryStore: remove transaction failed:"
                          << db.lastError().text();
        return;
    }
    ...
    q.addBindValue(imdbId);
    if (!q.exec()) {
        qCWarning(KINEMA_DB) << "LibraryStore: remove title failed:"
                          << q.lastError().text();
        db.rollback();
        return;
    }
    ...
    if (kind == domain::MediaKind::Series) {
        ...
        q.addBindValue(imdbId);
        if (!q.exec()) {
            qCWarning(KINEMA_DB) << "LibraryStore: remove episodes failed:"
                              << q.lastError().text();
            db.rollback();
            return;
        }
    ...
    if (!db.commit()) {
        qCWarning(KINEMA_DB) << "LibraryStore: remove commit failed:"
                          << db.lastError().text();
        db.rollback();
        return;
    }
