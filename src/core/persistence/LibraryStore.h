...
public:
    ...
    std::optional<domain::LibraryTitle> find(
        domain::MediaKind kind, const QString& imdbId) const;
    bool contains(domain::MediaKind kind, const QString& imdbId) const;
    ...
    std::optional<domain::LibraryEpisode> episode(
        const QString& imdbId, int season, int episode) const;

    void upsertTitle(const domain::LibraryTitle& title);
    ...
    void remove(domain::MediaKind kind, const QString& imdbId);
