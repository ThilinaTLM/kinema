...
public:
    ...
    QList<domain::DownloadItem> loadAll() const;
    ...
    /// `updated_at`.
    void upsert(const domain::DownloadItem& item);
    ...
    void updateMode(const QString& assetId, domain::DownloadMode mode);

    /// Toggle the cache disposition (pinned <-> ephemeral).
    void setDisposition(const QString& assetId, domain::CacheDisposition d);
    ...
    static StartArgs synthesiseStartArgs(const domain::DownloadItem& row);
