...
domain::HistoryEntry hydrate(const QSqlQuery& q)
{
...
void HistoryStore::runRetentionPass()
{
    ...
    if (q.numRowsAffected() > 0) {
        qCInfo(KINEMA_DB) << "HistoryStore: retention removed"
                       << q.numRowsAffected() << "finished rows";
        scheduleChanged();
    }
}

std::optional<domain::HistoryEntry> HistoryStore::find(
...
std::optional<domain::HistoryEntry> HistoryStore::findLatestForMedia(
...
QList<domain::HistoryEntry> HistoryStore::continueWatching(int limit) const
{
    QList<domain::HistoryEntry> out;
...
void HistoryStore::record(const domain::HistoryEntry& entry)
{
...
void HistoryStore::remove(const domain::PlaybackKey& key)
{
    if (!m_db.isOpen() || !key.isValid()) {
        return;
    }
...
void HistoryStore::setRememberedSubtitleLang(const domain::PlaybackKey& key,
    const QString& lang)
{
    if (!m_db.isOpen() || !key.isValid()) {
        return;
