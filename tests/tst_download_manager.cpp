...
private Q_SLOTS:
    ...
    void enqueueTorrentStartsBackgroundSession()
    {
        ...
        // background helper persists Downloading once the session
        // is realised; allow either Preparing or Downloading
        // depending on how many event-loop turns have elapsed.
        const auto ref = api::assetRefFor(stream, ctx);
        const auto row = m_store->find(api::assetIdFor(ref));
        QVERIFY(row.has_value());
        QVERIFY(row->state == api::DownloadState::Downloading
            || row->state == api::DownloadState::Preparing);
        QCOMPARE(row->disposition, api::CacheDisposition::Pinned);

        // Pinned ⇒ engine was told to keep the session alive.
