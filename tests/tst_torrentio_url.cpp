...
#include "api/torrentio/TorrentioUrl.h"
...
private Q_SLOTS:
    ...
    {
        const QUrl u(QStringLiteral("https://torrentio.strem.fun/resolve/alldebrid/"
        ...
        QCOMPARE(info->infoHash, QStringLiteral("255e2890a1a724bcdd93b1120dd08a5a8e11b949"));
        QCOMPARE(info->fileIndex, 20);
        QCOMPARE(info->fileNameHint, QStringLiteral("The Rookie S02E01.mkv"));
    }

    void parsesRealDebridResolveUrl()
    {
        const QUrl u(QStringLiteral("https://torrentio.strem.fun/resolve/realdebrid/"
                                    "ABCDEFGHIJKLMN/"
        ...
        const auto info = parseResolveUrl(u);
        QVERIFY(info.has_value());
        QCOMPARE(info->infoHash, QStringLiteral("aabb1122ccdd3344eeff5566778899aabbccddee"));
        QCOMPARE(info->fileIndex, 0);
        QCOMPARE(info->fileNameHint, QStringLiteral("Some.Movie.2024.1080p.mkv"));
    }

    void acceptsAlternateTorrentioMirrorHost()
    {
        const QUrl u(QStringLiteral("https://torrentio.example.com/resolve/alldebrid/TOK/"
    ...
    {
        const QUrl u(QStringLiteral("https://example.com/resolve/alldebrid/TOK/"
    ...
    {
        const QUrl u(QStringLiteral("https://torrentio.strem.fun/resolve/alldebrid/TOK/"
    ...
    {
        const QUrl u(QStringLiteral("https://torrentio.strem.fun/sort=seeders/stream/movie/"
                                    "tt0133093.json"));
        QVERIFY(!parseResolveUrl(u).has_value());
    }
    ...
    {
        const QUrl u(QStringLiteral("ftp://torrentio.strem.fun/resolve/alldebrid/TOK/"
                                    "aabb1122ccdd3344eeff5566778899aabbccddee/x.mkv/0/x.mkv"));
        QVERIFY(!parseResolveUrl(u).has_value());
    }
};
