...
#include "api/alldebrid/AllDebridParse.h"
...
private Q_SLOTS:
    ...
    void addMagnet_parses()
    {
        ...
        QCOMPARE(r.hash, QStringLiteral("842783e3005495d5d1637f5364b59343c7844707"));
        QVERIFY(r.ready);
    ...
    {
        const auto s = parseMagnetStatus(loadJsonFixture("ad_magnet_status_ready.json"));
        QCOMPARE(s.statusCode, 4);
        QCOMPARE(s.id, 123456LL);
        QCOMPARE(s.filename, QStringLiteral("ubuntu-18.04.2-live-server-amd64.iso"));
    }
    ...
    void magnetStatus_singleObjectShape_parses()
    {
        ...
        const auto s =
            parseMagnetStatus(loadJsonFixture("ad_magnet_status_ready_single_object.json"));
        QCOMPARE(s.id, 123456LL);
        QCOMPARE(s.statusCode, 4);
        QCOMPARE(s.filename, QStringLiteral("ubuntu-18.04.2-live-server-amd64.iso"));
    }
    ...
    {
        const auto s = parseMagnetStatus(loadJsonFixture("ad_magnet_status_downloading.json"));
    ...
    {
        const auto files = parseMagnetFiles(loadJsonFixture("ad_magnet_files_flat.json"));
        QCOMPARE(files.size(), 1);
        QCOMPARE(files.first().path, QStringLiteral("ubuntu-18.04.2-live-server-amd64.iso"));
        QCOMPARE(files.first().bytes, 875773970LL);
        QCOMPARE(files.first().hosterLink, QUrl(QStringLiteral("https://alldebrid.com/f/abcdefg")));
    }

    void magnetFiles_nested_flattenedWithJoinedPaths()
    {
        const auto files = parseMagnetFiles(loadJsonFixture("ad_magnet_files_nested.json"));
        ...
        QVERIFY(paths.contains(QStringLiteral("Show.S01.1080p/Show.S01E01.1080p.mkv")));
        QVERIFY(paths.contains(QStringLiteral("Show.S01.1080p/Show.S01E02.1080p.mkv")));
        QVERIFY(paths.contains(QStringLiteral("Show.S01.1080p/Subs/Show.S01E01.en.srt")));
    ...
    {
        const auto u = parseDelayed(loadJsonFixture("ad_delayed_processing.json"));
        QVERIFY(u.download.isEmpty());
    }
