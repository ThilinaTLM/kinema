...
private Q_SLOTS:
    void testSearchEncodesQueryIntoPath()
    {
        ...
        const auto results =
            QCoro::waitFor(client.search(MediaKind::Movie, QStringLiteral("the matrix")));
    ...
    {
        FakeHttpClient http;
        http.jsonReplies = {loadJsonFixture("cinemeta_meta_tt0133093.json"),
                            loadJsonFixture("cinemeta_meta_tt0903747_breaking_bad.json")};

        CinemetaClient client(&http);
        const auto movie =
            QCoro::waitFor(client.meta(MediaKind::Movie, QStringLiteral("tt0133093")));
        const auto series = QCoro::waitFor(client.seriesMeta(QStringLiteral("tt0903747")));
        ...
        QCOMPARE(http.calls.at(0).url.path(), QStringLiteral("/meta/movie/tt0133093.json"));
        QCOMPARE(http.calls.at(1).url.path(), QStringLiteral("/meta/series/tt0903747.json"));
    }
};
