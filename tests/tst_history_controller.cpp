...
private Q_SLOTS:
    ...
    //
    // The HistoryController disk write path moved to
    // `playback::progress::PlaybackProgressProjector`. The
    // session-end policy + chapter-derived credits-start + reason
    // mapping tests that used to live here are now covered by
    // `tst_playback_progress_projector.cpp` (driven through the
    // PlaybackEventStream rather than direct onEndOfFile calls).
    // HistoryController retains its in-memory state tracking for
    // the resume helper, which is exercised below.
